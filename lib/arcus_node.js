...
var PeerProxy = require('./peer_proxy.js');
...
var ArcusNode = module.exports = function(settings) {
  ...
  // Setup PeerProxy if given
  this.peerProxy = null;
  if(__p.settings.peerProxy !== false){

    __p.logger.debug('Setting up PeerProxy:', __p.settings.peerProxy);

    // Use ArcuseNode logger instance
    __p.settings.peerProxy.logger = __p.logger;

    this.peerProxy = PeerProxy.createServer(__p.settings.peerProxy);
        ...
        for(var peerNum = 0; peerNum < message.peerIds.length; peerNum++){
          ...
          if(peerNc)
          {
            ...
            if(__p.settings.peerProxy !== false && __p.peerRequestCount.get(requestCountKey).count > __p.settings.peerProxy.maxPeerRetry){
              ...
              __p.logger.debug('Rendezvouz request maximum reached, sending to PeerProxy at', _self.peerProxy.address(), 'for', remoteInfo, 'and', peerNc.endpoints[0]);

              _self.peerProxy.addPair(remoteInfo, peerNc.endpoints[0]);

              // Forward peer proxy address to peers
              newcomerMessage.address = _self.peerProxy.address();
              message.endpoints = [_self.peerProxy.address()];
            }
  ...
  this.run = function() {
    ...
    // Start PeerProxy if set
    if(__p.settings.peerProxy !== false){
      this.peerProxy.listen();
    }

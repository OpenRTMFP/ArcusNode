...
module.exports = function(settings) {
  ...
  _eventListeners[ArcusEvent.MESSAGE] = []; 
  ...
  var dispatchEvent = function(evt) {
    ...
    for(var i = 0; i < _eventListeners[type].length; i++) {
      var listener = _eventListeners[type][i];
    ...
    for(k in requests){
      ...
      switch(request.type){
        ...
        case RTMFP.REQUEST_TYPE.HANDSHAKE_1:
          ...
          var evt = new ArcusEvent(ArcusEvent.HANDSHAKE, _self, null, request);
          evt.data = {
            url: request.url,
        ...
        case RTMFP.REQUEST_TYPE.NET_CONNECTION_OPEN:
          ...
          var evt = new ArcusEvent(ArcusEvent.CONNECT, _self, nc, request);
          evt.data = request.messageData;
          dispatchEvent(evt);
          
          //Send ACK immediately
          _rtmfp.setResponse(responsePacket, { 
            type: RTMFP.REQUEST_TYPE.ACK, 
        ...
        case RTMFP.REQUEST_TYPE.MESSAGE:
          //emit message event here
          var evt = new ArcusEvent(ArcusEvent.MESSAGE, _self, nc, request);
          evt.command = request.messageName;
          evt.data = request.messageData;
          dispatchEvent(evt);
        ...
        case RTMFP.REQUEST_TYPE.NET_CONNECTION_CLOSE:
          if(nc.state() != NetConnection.CLOSING)
          {
            ...
            dispatchEvent(new ArcusEvent(ArcusEvent.DISCONNECT, _self, nc, request));
          }
          break;
   ...
   */
  this.finishRequest = function(request, nc) {
    ...
    switch(request.type){
      ...
      //
      case RTMFP.REQUEST_TYPE.MESSAGE:
      case RTMFP.REQUEST_TYPE.MESSAGE_RESULT:
      case RTMFP.REQUEST_TYPE.MESSAGE_ERROR:
  ...
  this.finishEvent = function(evt) {
    switch(evt.type()) {
      case ArcusEvent.HANDSHAKE:
        //TODO: allow forwarding with finish arguments
        if(typeof evt.finishData === 'undefined' || evt.finishData[0] !== false) {
          this.finishRequest(evt.request());
        }
        break;
      case ArcusEvent.MESSAGE:
        var request = evt.request();
        
        if(typeof evt.finishData === 'undefined') {
          //Let messages fail by default if there is no listener,
          //or event is not finished with a value
          request.type = RTMFP.REQUEST_TYPE.MESSAGE_ERROR;
          request.statusDescription = 'No message handler';
        } else if(evt.finishData[0] === false) {
          request.type = RTMFP.REQUEST_TYPE.MESSAGE_ERROR;
          
          //Add error description if given
          if(typeof evt.finishData[1] === 'string') {
            request.statusDescription = evt.finishData[1];
          }
        } else {
          request.type = RTMFP.REQUEST_TYPE.MESSAGE_RESULT;
          request.messageData = evt.finishData[0];
        }
        console.log('finishEvent MESSAGE ', request.type, evt.finishData);
        this.finishRequest(request, evt.nc());
        break;
      case ArcusEvent.CONNECT:
        this.finishRequest(evt.request(), evt.nc());
        break;
  ...
  var _sendClosePacket = function(nc) {
    if(nc.addresses.length > 0) {
      ...
      _rtmfp.setResponse(pkt, { type: RTMFP.REQUEST_TYPE.NET_CONNECTION_CLOSE });

...
var ArcusNode = module.exports = function(settings) {
  ...
  var _handleMessage = function(nc, message, remoteInfo, pkt) {
    ...
    switch(message.type){
      ...
      case RTMFP.HANDSHAKE_REQUEST:
        ...
        nc.addresses.push({ address: remoteInfo.address, port: remoteInfo.port });
    ...
    if(responsePacket){
      ...
      _self.send(responsePacket, (nc.addresses.length > 0) ? nc.addresses[0] : remoteInfo);
    }
  };
  ...
  var _closeConnection = function(nc){
    if(nc.state != NetConnection.CLOSING) {
      ...
      //Set NC state
      nc.state = NetConnection.CLOSING;
      nc.emit('close');
...
ArcusNode.prototype.pack = function(nc, packet){
  ...
  if(nc.state != NetConnection.HANDSHAKE){
    this.__p.logger.debug('Normal Packet to ', nc.addresses[0], ': \n' + packet.toString());
    this.__p.rtmfp.encryptPacket(packet, nc.__p.encryptKey);
  } else {
    this.__p.logger.debug('Handshake Packet to ', nc.addresses[0], ': \n' + packet.toString());
...
ArcusNode.prototype.failConnection = function(nc, flow){
  ...
  this.send(failPacket, nc.addresses[0]);
};

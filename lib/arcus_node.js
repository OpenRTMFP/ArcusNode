...
var ArcusNode = module.exports = function(settings) {
  ...
  _connections = {},
  _handshakeConnections = {},
  _connectionCounter = 0, //for temporary connection ids TODO: Replace with generated id
  _endpointConnections = {},
  _peerIdMap = {},
  _groups = {},
  _socket = null,
  _manageInterval = null,
  _commands = {},
  _running = false,
  _supportedProtocols = ['rtmfp:'],
  _startTime = 0;
  ...
  var _manageCycle = function(){
    try {
      ...
      for(k in _connections)
      {
        ...
        if((now - _connections[k].touched()) > _settings.serverKeepalive)
        {
          if(_connections[k].keepalives() < _settings.maxKeepalives)
          {
  ...
  var _handleMessage = function(nc, message, remoteInfo, pkt) {
    ...
    switch(message.type){
      ...
      case RTMFP.HANDSHAKE_REQUEST:
        ...
        if(_supportedProtocols.indexOf(url.protocol.toLowerCase()) == -1){
      ...
      case RTMFP.KEY_REQUEST: {
        ...
        if(nc)
        {
          ...
          var serverNonce = _rtmfp.createServerNonce(nc.getPublicKey());
      ...
      case RTMFP.NET_CONNECTION_ADDRESSES:
        ...
        if(nc.state() == NetConnection.CONNECTING)
        {
          ...
          message.serverKeepalive = _settings.serverKeepalive;
          message.clientKeepalive = _settings.clientKeepalive;
      ...
      case RTMFP.COMMAND:
        ...
        _sendResponse(message, nc, _rtmfp.writePacket(new Packet(32), { 
          type: RTMFP.ACK, 
      ...
      case RTMFP.RENDEZVOUZ_REQUEST:
        ...
        for(var peerNum = 0; peerNum < message.peerIds.length; peerNum++){
          peer = _peerIdMap[message.peerIds[peerNum]];
          if(peer)
          {
            ...
            _logger.debug('Rendezvouz requester response: ' + requesterResponse.toString());
  ...
  this.uptime = function(){
    return [(_startTime === 0) ? 0 : Math.floor((Date.now() - _startTime) * 0.001), process.uptime()];
  };
  ...
  this.status = function(){
    return (_socket !== null) ? 'listening' : 'not running';
  };

...
module.exports = function(settings) {
  ...
  var _manageCycle = function(){
    try {
      ...
      console.log('MANAGE CYCLE TIME: ' + (new Date().getTime() - now) + 'ms');
      console.log('#Connections: ' + counts[0] + ' #dropped NCs: ' + counts[1] + ' #handshake NCs: ' + counts[2] + ' #dropped hands. NCs: ' + counts[3]
        + ' #Groups: ' + counts[4] + ' #dropped groups: ' + counts[5]);
    } catch(e) {
      //TODO: handle error and recover
      console.error('Manage cycle error: %s ', e.stack);
    }
  };

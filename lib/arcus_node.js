...
var ArcusNode = module.exports = function(settings) {
  ...
  var _manageCycle = function(){
    try {
      ...
      __p.connections.forEach(function(id, connection)
      {
        //TODO: Touched may not be the last cable interaction for that connection (keep time in NC.__p.lastPacketReceived)
        if((now - connection.touched()) > __p.settings.serverKeepalive)
        {
          if(connection.keepalives() < __p.settings.maxKeepalives)
          {
            connection.keepalive();
            //TODO: Send server keepalive to client
          }
        }
        else if((now - connection.touched()) > __p.settings.connectionTimeout)
        {
          deadConnections.push(connection);
          connection.state(NetConnection.CLOSING);
        }
        else if(connection.lastMessage())
        {
          _resend(connection);
        }
      });
    ...
    switch(message.type){
      ...
      case RTMFP.RENDEZVOUZ_REQUEST:
        ...
        for(var peerNum = 0; peerNum < message.peerIds.length; peerNum++){
          peer = __p.peerIdMap.get(message.peerIds[peerNum]);
          if(peer)
          {

...
module.exports = function(settings) {
  ...
  var _packetHandler = function(buffer, remoteInfo){
    ...
    for(k in requests){
      ...
      switch(request.type){
        ...
        // COMMAND (RPC)
        //
        case RTMFP.REQUEST_TYPE.COMMAND:
          //emit command event here
          var evt = new ArcusEvent(ArcusEvent.COMMAND, _self, nc, request);
          evt.command = request.commandName;
          evt.data = request.commandData;
          dispatchEvent(evt);
  ...
  this.finishRequest = function(request, nc) {
    ...
    switch(request.type){
      ...
      //
      case RTMFP.REQUEST_TYPE.COMMAND:
      case RTMFP.REQUEST_TYPE.COMMAND_RESULT:
      case RTMFP.REQUEST_TYPE.COMMAND_ERROR:
    ...
    switch(evt.type()) {
      ...
      case ArcusEvent.COMMAND:
        ...
        if(typeof evt.finishData === 'undefined') {
          ...
          request.type = RTMFP.REQUEST_TYPE.COMMAND_ERROR;
          request.statusDescription = 'No command handler';
          ...
          request.type = RTMFP.REQUEST_TYPE.COMMAND_RESULT;
          request.commandData = evt.finishData[0];
        }

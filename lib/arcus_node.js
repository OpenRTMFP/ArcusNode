...
var ProxyNetConnection = require('./proxy_net_connection.js');
...
var ArcusNode = module.exports = function(settings) {
  ...
  __p.settings = {
    ...
    peerProxy: false, // If not false, an object with peer proxy settings: 
    /*
    {
      maxPeerRetry: 5
    }
  ...
  var _resend = function(nc) {
    if(nc.endpoints.length > 0)
    {
      ...
      _self.send(nc.socket, resendPacket, nc.endpoints[0]);
   ...
   */
  var _packetHandler = function(buffer, remoteInfo, socket){
    ...
    for(var i = 0; i < messages.length; i++){
      _handleMessage(nc, messages[i], remoteInfo, pkt, socket);
    }
  };
   ...
   */
  var _handleMessage = function(nc, message, remoteInfo, pkt, socket) {
    ...
    switch(message.type){
      ...
      case RTMFP.HANDSHAKE_REQUEST:
        ...
        nc = new NetConnection(_self, ++__p.connectionCounter, socket);
        
        //emit handshake event here
        _self.emit('handshake', nc, parsedUrl, remoteInfo);    
    
        _self._handle_handshakeRequest(nc, message, remoteInfo);
      ...
      case RTMFP.RENDEZVOUZ_REQUEST:
        ...
        for(var peerNum = 0; peerNum < message.peerIds.length; peerNum++){
          var requestedPeerId = message.peerIds[peerNum];
          peerNc = __p.peerIdMap.get(requestedPeerId);
          if(peerNc)
          {
            ...
            // TODO: Cleanup in management cycle
            var requestCountKey = remoteInfo.address + ':' + remoteInfo.port + ':' + requestedPeerId.toString('hex');
            if(__p.peerRequestCount.has(requestCountKey)){
              __p.peerRequestCount.get(requestCountKey).count++;
              __p.peerRequestCount.get(requestCountKey).touched = Date.now();
            } else {
              __p.peerRequestCount.add(requestCountKey, { count: 1, touched: Date.now() });
            }

            __p.logger.debug('Rendezvouz request count: ', requestCountKey, '-', __p.peerRequestCount.get(requestCountKey).count, '-', __p.settings.peerProxy.maxPeerRetry);

            // If PeerProxy is used and max peer requests are reached, use peer proxy
            if(__p.settings.peerProxy !== false && __p.peerRequestCount.get(requestCountKey).count > __p.settings.peerProxy.maxPeerRetry){

              __p.logger.debug('Rendezvouz request maximum reached, using peer proxy for', remoteInfo, 'and', peerNc.endpoints[0]);

              // Create peer proxy connection
              var nc = new ProxyNetConnection(_self, ++__p.connectionCounter, socket, {
                proxyPeerId: requestedPeerId
              });

              // Answer peer proxy connection request
              _self._handle_handshakeRequest(nc, message, remoteInfo);

              newcomerMessage.address = { address: __p.settings.publicAddress, port: __p.settings.port, public: true } || _self.address;
              ...
              __p.logger.debug('Rendezvouz requester response: \n' + requesterResponse.toString());
              __p.logger.debug('Rendezvouz requester response endpoints: ', message.endpoints);
              
              __p.rtmfp.encryptPacket(requesterResponse, RTMFP.SYMETRIC_KEY);
              __p.rtmfp.encodePacket(requesterResponse, 0);
              
              _self.send(socket, requesterResponse, remoteInfo);
            }            
            ...
            if(peerNc.endpoints.length > 0 && newcomerMessage.address != undefined)
            {
              ...
              __p.logger.debug('Rendezvouz newcomer response address: ', newcomerMessage.address);
              ...
              __p.logger.debug('send newcomer response to: ', peerNc.endpoints[0]);
              _self.send(peerNc.socket, directResponse, peerNc.endpoints[0]);
            }
    ...
    if(responsePacket){
      ...
      _self.send(nc ? nc.socket : socket, responsePacket, (nc.endpoints.length > 0) ? nc.endpoints[0] : remoteInfo);
    }
  };
   ...
   */ 
  this._handle_handshakeRequest = function(nc, message, remoteInfo){
    ...
    if(nc.state == NetConnection.HANDSHAKE){
      ...
      __p.rtmfp.writePacket(responsePacket, message);
      _self.pack(nc, responsePacket);
      _self.send(nc.socket, responsePacket, (nc.endpoints.length > 0) ? nc.endpoints[0] : remoteInfo);
    }
  }

  
  
  // END - Protocol request handlers
  //

  this.address = function() {
    if(!this._endpoint){
      this._endpoint = { address: this.settings.address || (this.socket != null ? this.socket.address().address : null), port: this.settings.port || (this.socket != null ? this.socket.address().port : null) };
    }
  ...
  this.commandResult = function(nc, message, result) {
    ...
    this.send(nc.socket, responsePacket, nc.endpoints[0]);
  };
  ...
  this.run = function() {
    ...
    //TODO: Support IPv6
    // TODO: Open several sockets on several ports if settings.port is an array with numbers
    // - hand over socket for packet and store socket with netconnection
    // - Send packets on netconnection directly where possible
    if(Array.isArray(__p.settings.port)){
      for(var i = 0; i < __p.settings.port.length; i++){
        this._bindSocket(__p.settings.port[i], _packetHandler);
      }
    } else {
      this._bindSocket(__p.settings.port, _packetHandler);
    }
  ...
  this._bindSocket = function(port, cb){

    __p.logger.debug('Binding socket on port ' + port);

    var socket = dgram.createSocket('udp4', function(buffer, remoteInfo){
      //Main Handler
      try {
        //TODO: call in arcus context
        cb(buffer, remoteInfo, socket);
      } catch(e) {
        //TODO: handle error and recover
        __p.logger.error('Socket message handler error on port ' + port + ': ' + e.stack, Packet.prettyBuffer(buffer));
      }
    });
    socket.bind(port, __p.settings.address);
    __p.sockets.add(port, socket);
  }
  ...
  this.stop = function() {
    ...
    if(__p.sockets.length > 0) {
      __p.sockets.forEach(function(index, socket){
        socket.close();
      });
    } 
    __p.sockets.wipe();
 ...
 */
ArcusNode.prototype.send = function(socket, packet, endpoint, callback) {
  ...
  if(this.__p.running) {
    socket.send(packet.buffer(), 0, packet.size(), endpoint.port, endpoint.address, function (err, bytes) {
        ...
        self.__p.logger.debug('Wrote ' + bytes + ' bytes to socket on port', socket.address().port);
...
ArcusNode.prototype.sendAck = function(nc, message, callback){
  ...
  this.pack(nc, pkt);
  this.send(nc.socket, pkt, nc.endpoints[0], callback);
}
...
ArcusNode.prototype.acceptConnection = function(nc, description){
  ...
  this.send(nc.socket, acceptPacket, nc.endpoints[0]);
...
ArcusNode.prototype.failConnection = function(nc, flow){
  ...
  this.send(nc.socket, failPacket, nc.endpoints[0]);
};
...
ArcusNode.prototype.__defineGetter__('status', function(){
  return (this.__p.sockets.length > 0) ? 'listening' : 'not running';
});
...
ArcusNode.prototype.__defineGetter__('address', function(){
  return this.__p.sockets.getAt(0).address();
});

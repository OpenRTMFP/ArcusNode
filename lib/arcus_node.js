...
module.exports = function(settings) {
  ...
  var _manageCycle = function(){
    try {
      ...
      _logger.debug('MANAGE CYCLE TIME: ' + (new Date().getTime() - now) + 'ms');
  ...
  var _messageHandler = function(buffer, remoteInfo){
    ...
    else if(!nc && connectionId == 0)
    {
      ...
      if(_settings.debug)
        _logger.info('Decrypted Handshake Packet: \n' + pkt.toString());
    } 
    else 
    {
    ...
    for(k in requests){
      ...
      switch(request.type){
        ...
        case RTMFP.REQUEST_TYPE.RENDEZVOUZ:
          ...
          if(peer)
          {
            _logger.info('Found requested peer for rendezvouz');
            ...
            if(peer.addresses.length > 0 && newcomerRequest.address != undefined)
            {
              ...
              _logger.info('Rendezvouz newcomer response: \n' + directResponse.toString());
      ...
      if(nc && responsePacket.size() > 11)
      {
        if(nc.state() != NetConnection.HANDSHAKE){
          if(_settings.debug)
            _logger.info('Normal Response to ' + remoteInfo.address + ':' + remoteInfo.port + ': \n' + responsePacket.toString());
          _rtmfp.encryptPacket(responsePacket, nc.encryptKey);
        } else {
          if(_settings.debug)
            _logger.info('Handshake Response to ' + remoteInfo.address + ':' + remoteInfo.port + ': \n' + responsePacket.toString());
  ...
  this.run = function() {
    //Startup
    _logger.info('Starting up ArcusNode RTMFP Service.\nCopyright (C) 2011 OpenRTMFP \n' +
      'This program comes with ABSOLUTELY NO WARRANTY.\n' +
      'This is free software, and you are welcome to redistribute it under certain conditions.');
  
    //Start manage cycle
    setInterval(_manageCycle, _settings.manageInterval * 1000);
    ...
    _logger.info('ArcusNode RTMFP Service running at ' + ((_settings.address != '') ? _settings.address + ':' : 'port ') + _settings.port);
  };
};
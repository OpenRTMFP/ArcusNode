...
var ArcusNode = module.exports = function(settings) {
  ...
  var _manageCycle = function(){
    try {
      ...
      {
        _self.closeConnection(deadConnections[i]);
      }
  ...
  var _handleMessage = function(nc, message, remoteInfo, pkt) {
    ...
    switch(message.type){
      ...
      case RTMFP.HANDSHAKE_REQUEST:
        ...
        nc.endpoints.push({ address: remoteInfo.address, port: remoteInfo.port });
      ...
      case RTMFP.NET_CONNECTION_ADDRESSES:
        ...
        //Add public address
        //TODO: save public/private marker with endpoints
        nc.endpoints.push(remoteInfo);
 ...
 */
ArcusNode.prototype.sendAck = function(nc, message){
 ...
 * TODO: Add callbacks for arcus api methods (when nc was actually closed)
 *
 * @param {NetConnection} nc
 * @return {ArcusNode} 
 */
ArcusNode.prototype.closeConnection = function(nc){
 ...
 */
ArcusNode.prototype.sendClosePacket = function(nc) {
  if(nc.endpoints.length > 0) {
    var pkt = new Packet(32);

    ...
    switch(type)
    {
      ...
      case 0x11 :
        ...
        if(request.sequence == 0x02)
        {
          ...
          request.commandName = AMF0.readString(clippedPkt);
          request.commandHandle = AMF0.readNumber(clippedPkt);
          ...
          switch(request.commandName) {
            ...
            case 'connect':
              ...
              request.commandData = AMF.readAMF0(clippedPkt);
              break;
            ...
            //read command object and return it with request
            default:
              //Read AMF Data
              request.type = types.COMMAND;
              //TODO: only read AMF data if null marker
              request.commandData = AMF.readAMF0(clippedPkt);
              break;
          }
  ...
  this.setResponse = function(pkt, request){
    ...
    switch(request.type)
    {
      ...
      case types.NET_CONNECTION_OPEN:
        ...
        AMF0.writeNumber(pkt, request.commandHandle);
        AMF0.writeNull(pkt);
      ...
      case types.COMMAND_ERROR:
        ...
        AMF0.writeString(pkt, (request.type === types.COMMAND_RESULT) ? '_result' : '_error');
        AMF0.writeNumber(pkt, request.commandHandle);
        
        //AMF0 NULL MARKER to close header
        AMF0.writeNull(pkt);
                    
        if(request.type === types.COMMAND_RESULT) {
          
          //write response AMF
          if(typeof request.commandData !== 'undefined'){
            AMF0.writeValue(pkt, request.commandData);
          }
          
        } else if(request.type === types.COMMAND_ERROR) {

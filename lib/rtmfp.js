...
RTMFP_VALID_MESSAGES = [0x10, 0x11, 0x30, 0x38, 0x51, 0x01, 0x41, 0x0c, 0x4c, 0x18, 0x71, 0x70, 0x78];
...
var RTMFP = module.exports = function(settings){
  ...
  var _nearCertificate = new Packet(new Buffer(64)).writeRandom(64).buffer();
  ...
  this.readMessage = function(pkt, lastMessage){
    ...
    switch(type)
    {
      ...
      case 0x38:
        ...
        if(keySize == 131)
        {
          console.log('handshake client key size wrong! (' + keySize + ')\n' + clippedPkt.toString());
        }
        
        var pos = clippedPkt.pos();
        
        message.clientSignature = clippedPkt.readBytes(sig_size);
        message.publicKey = clippedPkt.readBytes(keySize - sig_size);
  ...
  this.writePacket = function(pkt, message){
    ...
    switch(message.type)
    {
      ...
      case RTMFP.HANDSHAKE_RESPONSE:
        ...
        pkt.writeBuffer(_nearCertificate); //TODO: generate for each handshake
      ...
      case RTMFP.HANDSHAKE_REQUEST:
        ...
        pkt.writeInt16(_timeNow());
        pkt.writeInt8(0x30);
        pkt.writeInt16(13 + message.url.length);
        
        //write url size
        //TODO: write U29 length
        pkt.writeInt8(message.url.length);
        pkt.writeInt8(message.url.length - 1);
      ...
      case RTMFP.KEY_REQUEST:
        ...
        //TODO: write real keysize and signature size
        pkt.writeBytes([0x81, 0x04, 0x81, 0x02, 0x1d, 0x02]);
        
        // write faulty key and sig for testing
        pkt.writeBuffer(message.publicKey);
        //pkt.writeRandom(128);
        //var keyBuf = new Buffer(128);
        //keyBuf.write('', 'hex', 0, 128);
        //pkt.writeBuffer(keyBuf);
        
        pkt.writeBytes([0x4c, 0x02, 0x1d, 0x02, 0x41, 0x0e]);
        pkt.writeBuffer(_nearCertificate);
        pkt.writeBytes([0x03, 0x1a, 0x02, 0x0a, 0x02, 0x1e, 0x02]);
        
        pkt.writeInt8(0x58);
        
        //var certBuf = new Buffer(78);
        //certBuf.write('4C021D02410EA6E94491CF2B93B1DDAA7E35FA514A50FA7146FBE369976E1F735A155FA254F2E058F28F0F684E425C843A540C2DA074AB187255BAF45A24BA7149FDCBF77909031A020A021E0258', 'hex', 0, 78);
        //pkt.writeBuffer(certBuf);
   ...
   */
  this.createClientNonce = function(publicKey){
...
RTMFP.ACK = 0x10;
RTMFP.NOT_ACK = 0x11;
RTMFP.UNKNOWN_0x5e = 0x12;
RTMFP.COMMAND = 0x13;
RTMFP.COMMAND_RESULT = 0x14;
RTMFP.COMMAND_ERROR = 0x15;
RTMFP.FORWARD_REQUEST = 0x16;
RTMFP.FORWARD_RESPONSE = 0x17;

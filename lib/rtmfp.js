    ...
    switch(type)
    {
      ...
      case 0x30:
        ...
        else if(handshakeType == 0x0a)
        {
          ...
          //URL connected to
          request.url = pkt.readBytes(msgLength).toString('ascii');
        }
      ...
      case 0x11 :
        ...
        if(flag == 0x80) {
          request = {
            signature: pkt.readBytes(pkt.readInt8(), true)
          };
        } else {
          request = {};
        }
        ...
        if(sequence == 0x02)
        {
          ...
          request.messageName = AMF.readString(pkt);
          
          //? AMF Number ?
          var num = AMF.readNumber(pkt);
          ...
          switch(request.messageName) {
            ...
            case 'connect':
              ...
              //Read AMF Data
              request.messageData = AMF.read(pkt);
              console.log('NETCONNECTION DATA: ', request.messageData);
              break;
            ...
            default:
              //Read AMF Data
              request.messageData = AMF.read(pkt);
              console.log('Callback Message: ', request.messageName, request.messageData);
              break;
          }
        ...
        else if(sequence > 0x02 && stage == 0x01)
        {
          ...
          request.unknown1 = pkt.readBytes(2); //Unknown data
          pkt.skip(3);
          request.groupId = pkt.readBytes(pkt.read7Bit(), true);
        }

...
RTMFP_VALID_MARKERS = [0x8d, 0x0d, 0x0b, 0x89, 0x09, 0x49, 0x4e, 0x4a, 0x0e];
...
var RTMFP = module.exports = function(settings){
  ...
  this.readMessage = function(pkt, lastMessage){
    ...
    //Ensure packet bytes
    if(messageSize == 0 || pkt.available() < messageSize){
      return null;
    }
    ...
    switch(type)
    {
      ...
      case 0x30:
        ...
        if(message == null){
          throw new Error('Tried to read malicious handshake packet: ' + clippedPkt.toString());
        }
      ...
      //
      // Message: UNKNOWN_0x5e_REQUEST
      // TODO: rename, raises "NetConnection.Connect.Failed" on client, so its connection failure message
      //
      case 0x5e:
        message = { type: RTMFP.UNKNOWN_0x5e_REQUEST };
      ...
      case 0x11 :
        ...
        //TODO: Flag 0x03 is connect failed answer/retry without header (when connection request was acknowledged)
        //TODO: Flag 0x83 is connect failed answer/retry WITH header/signature (if connection request was NOT acknowledged
  ...
  this.writePacket = function(pkt, message){
    ...
    switch(message.type)
    {
      ...
      case RTMFP.HANDSHAKE_REQUEST:
        ...
        pkt.writeInt8(message.url.length + 2);
      ...
      case RTMFP.NET_CONNECTION_REQUEST:
        ...
        pkt.writeBytes([0x05, 0x00, 0x54, 0x43, 0x04, 0x00, 0x00, 0x14, 0x00, 0x00, 0x00, 0x00]); //hardcoded header
      ...
      //
      // UNKNOWN_0x5e_RESPONSE
      //
      case RTMFP.UNKNOWN_0x5e_RESPONSE:
      ...
      //
      // UNKNOWN_0x5e_REQUEST
      //
      case RTMFP.UNKNOWN_0x5e_REQUEST:
...
RTMFP.ACK = 0x10;
RTMFP.NOT_ACK = 0x11;
RTMFP.UNKNOWN_0x5e_REQUEST = 0x12;
RTMFP.UNKNOWN_0x5e_RESPONSE = 0x13;
RTMFP.COMMAND = 0x14;

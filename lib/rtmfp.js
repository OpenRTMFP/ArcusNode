    ...
    switch(type)
    {
      ...
      case 0x30:
        ...
        if(clippedPkt.peek() != 0x81)
        {
          ...
          clippedPkt.skip(2);
          msgLength = clippedPkt.read7Bit() - 1;
        }
      ...
      case 0x38:
        ...
        var keySize = clippedPkt.read7Bit();
      ...
      case 0x11 :
        ...
        if(request.sequence == 0x02)
        {
          ...
          request.messageName = AMF.readString(clippedPkt);
          
          //? AMF Number ?
          var num = AMF.readNumber(clippedPkt);
          ...
          switch(request.messageName) {
            ...
            case 'connect':
              ...
              //Read AMF Data
              request.messageData = AMF.read(clippedPkt);
              break;
        ...
        else if(request.sequence > 0x02 && request.stage == 0x01)
        {
          ...
          clippedPkt.skip(3);
          request.groupId = clippedPkt.readBytes(clippedPkt.read7Bit(), true);
        }

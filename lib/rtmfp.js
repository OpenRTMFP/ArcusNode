  ...
  this.getRequest = function(pkt){
    ...
    switch(type)
    {
      ...
      case 0x38:
        ...
        var key_size = pkt.read7Bit();
        
        var sig_size = 4;
        if(key_size == 131)
        {
          throw new Error('handshake client key size wrong!');
        }
        
        var pos = pkt.pos();
        
        request.clientSignature = pkt.readBytes(sig_size);
        request.clientKey = pkt.readBytes(key_size - sig_size);
        
        pkt.pos(pos);
        var keyPlusSig = pkt.readBytes(key_size);
        ...
        request.peerId = _rtmfp.computePeerId(keyPlusSig, key_size);

 ...
 */
var NetConnection = module.exports = function(arcus, id, socket, settings){
 ...
 */
NetConnection.prototype.init = function(arcus, id, socket, settings) {
  //Check ArcusNode instance
  if(typeof arcus !== 'object'){
    throw new Error('A NetConnection depends on an ArcusNode instance that has to be given in the init');
  }
  
  //Check id
  if(typeof id !== 'number'){
    throw new Error('A NetConnection need a numeric id that has to be given in the init');
  }

  //Check socket
  if(!socket){
    throw new Error('A NetConnection needs a socket it is attached to');
  }
...
NetConnection.prototype.generateKeyPair = function(){
  this.__p.arcus.__p.logger.debug('NC', this.id, ': generateKeyPair');
...
NetConnection.prototype.computeSharedSecret = function(publicKey){
  this.__p.arcus.__p.logger.debug('NC', this.id, ': computeSharedSecret');
...
NetConnection.prototype.computeAsymetricKeys = function(initiatorNonce, responderNonce){
  this.__p.arcus.__p.logger.debug('NC', this.id, ': computeAsymetricKeys');
 ...
 *
 * @param {Funcion} callback Is called after the connection was tried to close and gets a Boolean,
 * stating the success of the close operation.
 */
NetConnection.prototype.close = function(callback) {
  if(this.state != NetConnection.CLOSING) {
    this.state = NetConnection.CLOSING;
    this.__p.arcus.closeConnection(this, (function(err, bytes){
      var success = true;
      if(!err){
        ...
        // Handle the rare case when a connection could not be closed
        this.__p.state = NetConnection.CONNECTED;
        success = false;
      }

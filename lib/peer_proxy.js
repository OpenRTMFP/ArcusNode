  ...
  this.socket = dgram.createSocket('udp4', function(buffer, remoteInfo){
    ...
    try {
      ...
      if(self.settings.logPackets === true){
        self.logger.debug('PeerProxy packet:', remoteInfo, '\n', Packet.prettyBuffer(buffer));  
      }
...
PeerProxy.prototype.address = function() {
  if(!this._endpoint){
    this._endpoint = { address: this.settings.publicAddress || this.socket.address().address, port: this.socket.address().port };
  }

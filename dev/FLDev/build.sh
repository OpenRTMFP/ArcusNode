...
rm -f "$BUILD_PATH/bin/fldev.swf"

mxmlc -output="$BUILD_PATH/bin/fldev.swf" -source-path+="$BUILD_PATH/lib" --target-player=11.2 -debug -swf-version 16 "$BUILD_PATH/src/Main.as" -static-link-runtime-shared-libraries=true